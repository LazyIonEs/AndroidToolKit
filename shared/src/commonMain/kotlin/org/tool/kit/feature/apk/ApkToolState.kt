...
data class ApkToolForm(
    ...
    val icon: String = "",
    val packageName: String = "org.apk.tool",
    val targetSdkVersion: String = "30",
    val minSdkVersion: String = "21",
    val versionCode: String = "1",
    val versionName: String = "1.0",
    val appName: String = "HelloAndroid",
    val enableSign: Boolean = false,
    val policy: SignaturePolicy = SignaturePolicy.V3,
...
sealed interface ApkToolIntent {
    ...
    data class StorePasswordChanged(val value: String) : ApkToolIntent {
        override fun toString() = "StorePasswordChanged(redacted)"
    }

    data class AliasChanged(val index: Int) : ApkToolIntent
    data class AliasPasswordChanged(val value: String) : ApkToolIntent {
        override fun toString() = "AliasPasswordChanged(redacted)"
    }
...
object ApkToolFormReducer {
    ...
    private fun SigningCredentialsUi.path(value: String) =
        if (path == value) this else SigningCredentialsUi(path = value)
    ...
    fun field(form: ApkToolForm, intent: ApkToolIntent): ApkToolForm = when (intent) {
        ...
        is ApkToolIntent.TargetSdkChanged -> if (intent.value.acceptedNumber()) form.copy(
            targetSdkVersion = intent.value
        ) else form

        is ApkToolIntent.MinSdkChanged -> if (intent.value.acceptedNumber()) form.copy(minSdkVersion = intent.value) else form
        is ApkToolIntent.VersionCodeChanged -> if (intent.value.acceptedNumber()) form.copy(
            versionCode = intent.value
        ) else form
        ...
        is ApkToolIntent.EnableSignChanged -> form.copy(
            enableSign = intent.value,
            credentials = form.credentials.path("")
        )

        is ApkToolIntent.KeyPathChanged -> form.copy(credentials = form.credentials.path(intent.value))
        is ApkToolIntent.StorePasswordChanged -> form.copy(
            credentials = form.credentials.copy(
                storePassword = intent.value
            )
        )

        is ApkToolIntent.AliasChanged -> form.copy(
            credentials = form.credentials.copy(
                aliasIndex = intent.index,
                aliasPassword = if (intent.index == form.credentials.aliasIndex) form.credentials.aliasPassword else ""
            )
        )

        is ApkToolIntent.AliasPasswordChanged -> form.copy(
            credentials = form.credentials.copy(
                aliasPassword = intent.value
            )
        )
