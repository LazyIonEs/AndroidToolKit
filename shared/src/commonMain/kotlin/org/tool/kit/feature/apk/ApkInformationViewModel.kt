            ...
            is ApkInformationIntent.ReadApk -> {
                ...
                request.launch(block = {
                    try {
                        ...
                        Result.success(ApkInformationResultUi(data.label, data.icon?.let { decoder.decode(it) },
                            ...
                            data.usesPermissionList?.toList(), data.nativeCode, data.channel))
                    } catch (cancelled: CancellationException) { throw cancelled }
                    catch (error: Exception) { Result.failure(error) }
                }) { outcome ->
                    outcome.fold(onSuccess = { result ->
                        _uiState.update { it.copy(phase = ApkInformationPhase.Result, result = result) }
                    }, onFailure = { error ->
                        _uiState.update { it.copy(phase = ApkInformationPhase.Idle, result = null) }
                        val message = if (error is ApkCommandFailed) UiMessage.Resource(Res.string.exec_command_error)
                            else error.message?.let(UiMessage::Text) ?: UiMessage.Resource(Res.string.apk_parsing_failed)
                        viewModelScope.launch { effects.send("apk-information", SnackbarMessage(message), id) }
                    })
                }
            }
            is ApkInformationIntent.CopyText -> {
                ...
                viewModelScope.launch {
                    try {
                        ...
                        effects.send("apk-information", SnackbarMessage(UiMessage.Resource(Res.string.copied_to_clipboard)), id)
                    } catch (cancelled: CancellationException) { throw cancelled }
                    catch (error: Exception) {
                        io.github.oshai.kotlinlogging.KotlinLogging.logger("ApkInformation").error(error) { "Clipboard write failed" }
                    }
                }
            }
        }
