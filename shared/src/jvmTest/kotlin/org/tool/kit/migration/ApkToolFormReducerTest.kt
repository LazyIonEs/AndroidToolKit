...
class ApkToolFormReducerTest {
    @Test fun defaultsNumericFiltersAndEveryTextFieldMatchTheLegacyForm() {
    ...
    @Test fun keyAndToggleResetRulesMatchTheFrozenSignSetterIncludingAnAlreadyEmptyPath() {
        for (oldPath in listOf("", "old.jks")) for (newPath in listOf("", "old.jks", "new.jks")) {
            val legacy = LegacySign(_keyStorePath = oldPath, keyStorePassword = "store",
                keyStoreAlisaList = arrayListOf("a", "b"), keyStoreAlisaIndex = 1, keyStoreAlisaPassword = "key")
            val form = ApkToolForm(credentials = SigningCredentialsUi(oldPath, "store", listOf("a", "b"), 1, "key"))
            legacy.keyStorePath = newPath
            val actual = ApkToolFormReducer.field(form, KeyPathChanged(newPath)).credentials
            assertEquals(SigningCredentialsUi(legacy.keyStorePath, legacy.keyStorePassword, legacy.keyStoreAlisaList,
                legacy.keyStoreAlisaIndex, legacy.keyStoreAlisaPassword), actual)
