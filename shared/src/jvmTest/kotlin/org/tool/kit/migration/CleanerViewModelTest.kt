...
class CleanerViewModelTest {
    ...
    @Test fun confirmationUsesTheLatestSelectionAndCancelledOrStaleConfirmationNeverDeletes() = runTest {
        val f = CleanerVmFixture(StandardTestDispatcher(testScheduler)); val messages = mutableListOf<AppEffect>()
        ...
        try {
            ...
            assertEquals(UiMessage.Resource(Res.string.select_delete_director), messages.single().snackbar.message)
            ...
            withTimeout(5_000) { f.effectsPending(messages, Res.string.cleanup_complete, 12L.formatFileSize()) }
    ...
    @Test fun deletionRemovesSuccessesProgressivelyRetainsFailuresAndCountsScannedBytes() = runTest {
        ...
        try {
            ...
            val expected = UiMessage.Text(getString(Res.string.file_deletion_exception, 1))
            withTimeout(5_000) { while (messages.none { it.snackbar.message == expected }) yield() }
            ...
            withTimeout(5_000) { f.effectsPending(messages, Res.string.cleanup_complete, 30L.formatFileSize()) }
    ...
    @Test fun shellPredicatesMatchEveryPhasePageListAndOtherOperationCombination() {
        for (phase in CleanerPhase.entries) for (items in listOf(false, true)) for (page in listOf(false, true)) for (other in listOf(false, true)) {
            val result = cleanerShellVisibility(CleanerShellState(items, phase), page, other)
            assertEquals(!items, result.showNavigationRail)
            assertEquals(page && phase == CleanerPhase.Idle && items, result.showBottomBar)
            assertEquals(other || phase == CleanerPhase.Deleting, result.showGlobalLoading)
        }
    }
...
private class CleanerVmFixture(dispatcher: TestDispatcher, repository: BuildCachesRepository? = null, storage: StorageRepository? = null) {
    ...
    suspend fun effectsPending(messages: List<AppEffect>, resource: org.jetbrains.compose.resources.StringResource, value: String) {
        val expected = UiMessage.Text(getString(resource, value))
        while (messages.none { it.snackbar.message == expected }) yield()
    }
