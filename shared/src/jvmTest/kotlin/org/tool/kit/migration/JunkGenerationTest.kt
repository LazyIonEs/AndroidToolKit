...
class JunkGenerationTest {
    ...
    @Test fun randomBatchUsesInclusiveBoundsAndRetriesDuplicatePackagesAndPrefixes() {
        ...
        assertEquals(1, configs.map { it.packageName.replace(".", "") }.distinct().size, "Distinct valid names can collide in the legacy workspace")
    ...
    @Test fun realSingleAndBatchArchivesPreserveNamingStructureAndSizeRules() = runBlocking {
        ...
        // The old generator rejects zero activities; failure must still clean only its own workspace.
        ...
        val evidence = File("build/migration/fixtures/phase7b-result.txt")
        evidence.parentFile.mkdirs(); evidence.writeText("PASS: one single and two batch AARs; ASM classes, manifest, resources, original naming and measured size; private workspace cleanup and unrelated paths preserved\n")
    }
