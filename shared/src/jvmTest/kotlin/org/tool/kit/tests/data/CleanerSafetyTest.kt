...
class CleanerSafetyTest {
    ...
    private val logs = CleanerRuleGroup(
        "logs",
        "Logs",
    ...
    @Test
    fun hiddenDepthDedupAndParentPruningApplyToBothTargetTypes() = runBlocking {
        ...
        val request = CleanerScanRequest.from(
            root.path,
            CleanerRuleConfig(
                maxDepth = 1,
                includeHidden = false,
                rules = listOf(defaultBuildRule(), logs, logs.copy(id = "other"))
            )
        )
        ...
        assertEquals(
            setOf("build", "a.log", ".hidden.log", ".hidden/build", "outer/build"),
            hidden.map { it.displayPath }.toSet()
        )
        assertEquals(hidden.size, hidden.map { it.path }.distinct().size)
    }

    @Test
    fun changingToLinksFilesMissingAndOutsideAncestorsFailsSafely() = runBlocking {
        ...
        gone.delete()
        ancestor.delete(); ancestor.parentFile.delete(); Files.createSymbolicLink(
        ancestor.parentFile.toPath(),
        outside.toPath()
    )
    ...
    @Test
    fun directoryDeletionUnlinksNestedSymlinksWithoutTouchingTargets() = runBlocking {
        val root = temporary.newFolder("scan")
        val outside =
            temporary.newFolder("outside").apply { resolve("keep").writeBytes(ByteArray(4000)) }
    ...
    @Test
    fun changedFileSizeIsReevaluatedFromTheScannedRules() = runBlocking {
        val file = temporary.root.resolve("a.log").apply { writeBytes(ByteArray(1025)) }
        val rule = logs.copy(
            conditions = logs.conditions + CleanerCondition.FileSizeGreaterThan(
                1024,
                "1",
                SizeUnit.KB
            )
        ...
        val item = repo.scan(
            CleanerScanRequest.from(
                temporary.root.path,
                CleanerRuleConfig(rules = listOf(rule))
            )
        ).single()
    ...
    @Test
    fun inaccessibleEntriesAreNonFatalAndOnlyMatchedDirectoriesAreMeasured() = runBlocking {
        val root = temporary.root
        val blocked =
            root.resolve("blocked").apply { mkdirs(); resolve("inner").writeText("secret") }
        ...
        try {
            ...
            assertEquals(
                listOf(good.canonicalPath),
                repo.scan(depthOne, issues::add).toList().map { it.path })
            assertTrue(
                issues.isEmpty(),
                "Unmatched directories at the depth limit must not be measured"
            )
            repo.scan(depthOne.copy(maxDepth = 2), issues::add).toList()
            assertTrue(issues.contains(blocked.canonicalPath))
            issues.clear()
            val matchBlocked =
                defaultBuildRule().copy(conditions = listOf(CleanerCondition.Text(value = "blocked")))
            assertEquals(
                listOf(blocked.canonicalPath),
                repo.scan(depthOne.copy(rules = listOf(matchBlocked)), issues::add).toList()
                    .map { it.path })
            assertTrue(
                issues.contains(blocked.canonicalPath),
                "Matched directory measurement reports access failures"
            )
        } finally {
            Files.setPosixFilePermissions(blocked.toPath(), old)
        }
    }

    @Test
    fun fileRulesDoNotMeasureDirectoriesAndCancelledScanEmitsNoLateResults() = runBlocking {
        val root = temporary.root
        ...
        job.join(); delay(30.milliseconds)
        assertEquals(1, results.size); assertFalse(results.single().isDirectory)
        assertEquals(40, repo.scan(request).toList().size)
    }

    @Test
    fun rootMustExistAndBeARealAccessibleDirectory() = runBlocking {
        assertFailsWith<IllegalArgumentException> {
            repo.scan(temporary.root.resolve("missing").path).toList()
        }
