...
class CleanerRulesUiTest {
    @Test fun onlyHeadersToggleSectionsAndExpandedContentKeepsItsOwnActions() = runDesktopComposeUiTest(width = 800, height = 1000) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        runBlocking { repo.save(CleanerRuleConfig(rules = listOf(defaultBuildRule()))) }
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
            runOnIdle { vm.open() }
            setContent { AppTheme(false) { val state by vm.uiState.collectAsState(); CleanerRulesScreen(state, vm, {}) } }
            ...
            fun headerClick() = header.performMouseInput { click(androidx.compose.ui.geometry.Offset(24f, centerY)) }
            ...
            card.performMouseInput { click(androidx.compose.ui.geometry.Offset(12f, height - 28f)) }
            options.assertIsDisplayed()
            ...
            optionsBody.performMouseInput { click(androidx.compose.ui.geometry.Offset(width - 4f, height - 20f)) }
            ...
            settings.performMouseInput { click(androidx.compose.ui.geometry.Offset(4f, centerY)) }
    ...
    @Test fun wholeRuleChangesAnimateNeighborAndScanSettingsPositions() = runDesktopComposeUiTest(width = 800, height = 1200) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        runBlocking { repo.save(CleanerRuleConfig(rules = listOf(defaultBuildRule(),
            defaultBuildRule().copy(id = "second", name = "第二条", conditions = listOf(CleanerCondition.Text(value = "cache"))),
            defaultBuildRule().copy(id = "third", name = "第三条", conditions = listOf(CleanerCondition.Text(value = "temp")))))) }
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
            runOnIdle { vm.open() }
            ...
            fun checkMovedThroughIntermediatePositions(start: Float, samples: List<Float>, downward: Boolean) {
                val end = samples.last()
                assertTrue(if (downward) end > start + 2f else end < start - 2f, "$start -> $samples")
                assertTrue(samples.any { it > minOf(start, end) + 1f && it < maxOf(start, end) - 1f },
            ...
            checkMovedThroughIntermediatePositions(beforeDelete, settingsPositions, downward = false)
            checkMovedThroughIntermediatePositions(beforeNeighbor, neighborPositions, downward = false)
        } finally { mainClock.autoAdvance = true; store.clear() }
    }

    @Test fun connectedChoicesKeepBoundsStableThroughCheckmarkExit() = runDesktopComposeUiTest(width = 800, height = 800) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        runBlocking { repo.save(CleanerRuleConfig(rules = listOf(defaultBuildRule().copy(target = CleanerTarget.FILE,
            conditions = listOf(CleanerCondition.Text(value = "build"), CleanerCondition.Text(value = "cache")))))) }
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
            runOnIdle { vm.open() }
            ...
            for ((tag, choices) in listOf("rule-target-$ruleId" to listOf("文件夹", "文件"),
                ...
                for (choice in choices) {
                    onNode(hasText(choice, substring = false) and hasAnyAncestor(hasTestTag(tag)), useUnmergedTree = true).performClick()
                    val label = onNode(hasText(choice, substring = false) and hasAnyAncestor(hasTestTag(tag)), useUnmergedTree = true)
                    ...
                    assertTrue(labelLefts.drop(20).zipWithNext().all { (a, b) -> kotlin.math.abs(b - a) <= 2f },
                        "$choice label jumps near the end of its animation: $labelLefts")
                    assertTrue(widths.all { kotlin.math.abs(it - initial.width) <= 1f },
    ...
    @Test fun inputMethodSeesChineseCompositionAndCursorBeforeTheNextFrame() = runDesktopComposeUiTest(width = 800, height = 900) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        ...
        try {
            runOnIdle { vm.open() }
            setContent {
                InterceptPlatformTextInput({ request, _ -> input = request; awaitCancellation() }) {
                    AppTheme(false) { val state by vm.uiState.collectAsState(); CleanerRulesScreen(heldDraft ?: state, vm, {}) }
                }
            }
            waitUntil { !vm.uiState.value.loading }
            onNodeWithContentDescription("展开规则").performClick()
            ...
            for (label in listOf("规则名称", "条件值")) {
                ...
                runOnIdle {
                    ...
                    request.onEditCommand(listOf(SetSelectionCommand(1, 1), CommitTextCommand("临时", 1)))
                    assertEquals("缓临时存", request.state.text)
                    assertEquals(TextRange(3), request.state.selection)
                    request.onEditCommand(listOf(SetSelectionCommand(0, 4), CommitTextCommand("中文缓存", 1)))
            ...
            assertEquals("中文缓存", (vm.uiState.value.draft.rules.single().conditions.single() as CleanerCondition.Text).value)
            assertEquals("build", (repo.state.value.config.rules.single().conditions.single() as CleanerCondition.Text).value)
        } finally { store.clear() }
    }

    @Test fun numericInputsKeepSelectionAndRestoreUpdatesTheVisibleBuffer() = runDesktopComposeUiTest(width = 800, height = 1000) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        runBlocking { repo.save(CleanerRuleConfig(rules = listOf(defaultBuildRule().copy(target = CleanerTarget.FILE,
            conditions = listOf(CleanerCondition.FileSizeGreaterThan(1024, "1", SizeUnit.KB)))))) }
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        var input: PlatformTextInputMethodRequest? = null
        try {
            ...
            setContent {
                InterceptPlatformTextInput({ request, _ -> input = request; awaitCancellation() }) {
                    AppTheme(false) { val state by vm.uiState.collectAsState(); CleanerRulesScreen(state, vm, {}) }
                }
            }
            waitUntil { !vm.uiState.value.loading }
            onNodeWithContentDescription("展开规则").performClick()
            val rowId = vm.uiState.value.conditionRows.getValue(defaultBuildRule().id).single().id
            onNodeWithTag("condition-edit-$rowId").performClick()
            onNodeWithText("超过", substring = false).performTextClearance()
            runOnIdle {
                val request = checkNotNull(input)
                for (digit in "128") request.onEditCommand(listOf(CommitTextCommand(digit.toString(), 1)))
                assertEquals("128", request.state.text)
                assertEquals(TextRange(3), request.state.selection)
                request.onEditCommand(listOf(SetSelectionCommand(1, 2), CommitTextCommand("0", 1)))
            ...
            val size = vm.uiState.value.draft.rules.single().conditions.single() as CleanerCondition.FileSizeGreaterThan
            assertEquals("108", size.displayValue)
            assertEquals(108L * 1024 * 1024, size.bytes)
            onNodeWithTag("cleaner-scan-settings-header", useUnmergedTree = true).performScrollTo().performClick()
    ...
    @Test fun conditionEditorHeightAnimatesInBothDirectionsWithoutLosingInput() = runDesktopComposeUiTest(width = 800, height = 800) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
            ...
            assertEquals("retained-cache", (vm.uiState.value.draft.rules.single().conditions.single() as CleanerCondition.Text).value)
        } finally { mainClock.autoAdvance = true; store.clear() }
    }

    @Test fun allExistingRulesStartCollapsedAndEditingFollowsOnlyTheOpenRule() = runDesktopComposeUiTest(width = 800, height = 800) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        runBlocking { repo.save(CleanerRuleConfig(rules = listOf(defaultBuildRule(),
            CleanerRuleGroup("disabled", "临时文件", enabled = false, target = CleanerTarget.FILE,
                conditions = listOf(CleanerCondition.Text(operator = TextOperator.ENDS_WITH, value = ".tmp")))))) }
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
            runOnIdle { vm.open() }
            ...
            onNode(hasContentDescription("展开规则") and hasAnyAncestor(hasTestTag("rule-disabled"))).performScrollTo().performClick()
            onAllNodesWithContentDescription("折叠规则").assertCountEquals(1)
            onAllNodes(hasSetTextAction()).assertCountEquals(0)
            assertEquals("edited-build", (vm.uiState.value.draft.rules.first().conditions.single() as CleanerCondition.Text).value)
            assertEquals("build", (repo.state.value.config.rules.first().conditions.single() as CleanerCondition.Text).value)
            onNodeWithText("添加规则", substring = false).performClick()
            val added = vm.uiState.value.draft.rules.last()
            onAllNodesWithContentDescription("折叠规则").assertCountEquals(1)
            onNodeWithText("条件值", substring = false).performScrollTo().performTextReplacement("cache")
            onNode(hasContentDescription("规则操作") and hasAnyAncestor(hasTestTag("rule-${added.id}"))).performScrollTo().performClick()
    ...
    @Test fun materialEditorFiltersSizeFieldsAndConfirmsDestructiveTargetChanges() = runDesktopComposeUiTest(width = 1000, height = 900) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
    ...
    @Test fun restoreOnlyChangesDraftAndSaveTryRunPublishesSuccess() = runDesktopComposeUiTest(width = 1000, height = 900) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
            ...
            waitUntil { vm.uiState.value.saved }
            assertTrue(vm.uiState.value.tryRun); assertEquals(1, repo.state.value.config.revision)
        } finally { store.clear() }
    }

    @Test fun resultExplainsBothTypesAndDeletionShowsFrozenCounts() = runDesktopComposeUiTest(width = 1000, height = 700) {
        val log = CleanerItemUi("file", "/root/a.log", "a.log", 1024, 0, false, true, matchedRuleNames = listOf("大型日志", "临时文件"))
        val folder = CleanerItemUi("folder", "/root/build", "build", 2048, 0, true, true, matchedRuleNames = listOf("Android 构建缓存"), deleteFailed = true, safetyFailure = true)
        var state by mutableStateOf(CleanerUiState(scanRoot = "/root", items = listOf(log, folder)))
        setContent { AppTheme(false) { CleanerScreen(state, { if (it == CleanerIntent.RequestDelete) state = state.copy(deleteConfirmVisible = true, confirmationItems = state.items) }, {}, {}) } }
    ...
    @Test fun scanningCountsDiscoveriesSeparatelyFromSelectionAndAllowsCancellation() = runDesktopComposeUiTest(width = 800, height = 572) {
        val item = CleanerItemUi("file", "/root/a.log", "a.log", 1024, 0, false, true, checked = false, matchedRuleNames = listOf("Logs"))
        val intents = mutableListOf<CleanerIntent>()
        setContent { AppTheme(false) { CleanerScreen(CleanerUiState(phase = CleanerPhase.Scanning, items = listOf(item)), intents::add, {}, {}) } }
    ...
    @Test fun saveTryRunReturnsToCleanerRequestsPickerAndScansWithCommittedRules() = runDesktopComposeUiTest(width = 800, height = 572) {
        ...
        val repo = object : BuildCachesRepository {
            ...
            override fun scan(request: CleanerScanRequest, onIssue: (String) -> Unit): Flow<BuildDirectory> {
                requests += request
                return flowOf(BuildDirectory(request.root, "${request.root}/build", "build", 10, 0, true, true))
            }
        ...
        val cleaner = CleanerViewModel(ScanBuildCachesUseCase(repo), DeleteBuildCachesUseCase(repo), AllPathsExist, effects, rules = rules)
        val editor = CleanerRulesViewModel(rules)
        val store = ViewModelStore().also { it.put("cleaner", cleaner); it.put("rules", editor) }
        var pickerRequests = 0
        try {
            setContent { AppTheme(false) { CleanerContent(cleaner, editor, false, {
                pickerRequests++
        ...
        } finally { store.clear(); effects.close() }
    }

    @Test fun toolbarWindowKeepsResultsAndCancelDiscardsDraft() = runDesktopComposeUiTest(width = 800, height = 650) {
        val rules = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        val repo = object : BuildCachesRepository {
            override fun scan(root: String) = flowOf(BuildDirectory(root, "$root/build", "build", 10, 0, true, true))
            override suspend fun delete(directory: BuildDirectory): DeleteBuildCacheResult = error("Unexpected deletion")
        }
        val effects = AppEffectSink()
        val cleaner = CleanerViewModel(ScanBuildCachesUseCase(repo), DeleteBuildCachesUseCase(repo), AllPathsExist, effects, rules = rules)
        val editor = CleanerRulesViewModel(rules)
        val store = ViewModelStore().also { it.put("cleaner", cleaner); it.put("editor", editor) }
        try {
            setContent { AppTheme(false) { CleanerContent(cleaner, editor, false, {}, {},
                rulesWindow = { state, dismiss, saved, _ -> CleanerRulesWindowContent(state, editor, dismiss, saved) }) } }
            waitUntil { cleaner.uiState.value.rulesReady }
            ...
            val contentBounds = onNodeWithTag("cleaner-rules-window-content").fetchSemanticsNode().boundsInRoot
            assertTrue(kotlin.math.abs(contentBounds.bottom - 650f) <= 1f, "Editor bottom: $contentBounds")
            ...
            waitUntil { onAllNodesWithTag("cleaner-rules-window-content").fetchSemanticsNodes().isEmpty() }
            ...
            waitUntil { !editor.uiState.value.loading }
            assertEquals(defaultBuildRule().name, editor.uiState.value.draft.rules.single().name)
        } finally { store.clear(); effects.close() }
    }

    @Test fun collapsibleSectionsKeepEditsAndSavingBlocksWindowDismissal() = runDesktopComposeUiTest(width = 850, height = 850) {
        ...
        try {
            ...
            setContent { AppTheme(false) {
                ...
                CleanerRulesWindowContent(state, editor, { dismissed = true }, { saved = true })
            } }
            waitUntil { !editor.uiState.value.loading }
            val ruleId = defaultBuildRule().id
            ...
            onNodeWithTag("cleaner-rules-window-content").performMouseInput { click(androidx.compose.ui.geometry.Offset(4f, 4f)) }
            // Card padding is inert; only the header and arrow advertise expansion.
            onNodeWithTag("rule-$ruleId").performMouseInput { click(androidx.compose.ui.geometry.Offset(8f, height - 8f)) }
            onNodeWithTag("rule-rename-$ruleId").assertDoesNotExist()
            onNodeWithTag("rule-header-$ruleId").performMouseInput { click(androidx.compose.ui.geometry.Offset(24f, centerY)) }
            onNodeWithTag("rule-rename-$ruleId").performClick()
            onNodeWithText("规则名称").performTextReplacement("Retained name")
            onNodeWithTag("rule-header-$ruleId").performMouseInput { click(androidx.compose.ui.geometry.Offset(24f, centerY)) }
            onNodeWithText("规则名称").assertTextContains("Retained name")
            onNodeWithContentDescription("完成编辑").performClick()
            onNodeWithTag("rule-header-$ruleId", useUnmergedTree = true).performMouseInput { click(androidx.compose.ui.geometry.Offset(24f, centerY)) }
            onNodeWithText("规则名称").assertDoesNotExist()
            onNodeWithContentDescription("展开规则").performClick()
        ...
        } finally { gate.complete(Unit); store.clear() }
    }

    @Test fun summaryMovesSmoothlyThroughFinalCollapseFrames() = runDesktopComposeUiTest(width = 1000, height = 1050) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
            ...
            for (target in CleanerTarget.entries) {
                ...
                val summary = onNodeWithTag("rule-summary-${defaultBuildRule().id}", useUnmergedTree = true)
                ...
                val tail = positions.zipWithNext().filter { (before, _) -> kotlin.math.abs(before - restingTop) <= 16f }
                assertTrue(tail.isNotEmpty())
                assertTrue(tail.all { (before, after) -> kotlin.math.abs(after - before) <= 4f },
    ...
    @Test fun conditionRemovalAnimatesAndKeepsRemainingRowEditsInPlace() = runDesktopComposeUiTest(width = 1000, height = 1050) {
        val repo = DefaultCleanerRulesRepository(MapSettings().toFlowSettings(Dispatchers.Unconfined))
        val vm = CleanerRulesViewModel(repo)
        val store = ViewModelStore().also { it.put("rules", vm) }
        try {
            ...
            waitUntil { !vm.uiState.value.loading }
            runOnIdle {
                vm.updateRule(defaultBuildRule().copy(conditions = listOf(
                    CleanerCondition.Text(value = "one"), CleanerCondition.Text(value = "two"), CleanerCondition.Text(value = "three"))))
            }
            onNodeWithContentDescription("展开规则").performClick()
            val ruleId = defaultBuildRule().id
            ...
            waitForIdle()
            mainClock.autoAdvance = false
            onNode(hasContentDescription("删除条件") and hasAnyAncestor(hasTestTag("condition-${rows[1].id}")), useUnmergedTree = true).performClick()
            mainClock.advanceTimeBy(32)
            onNodeWithTag("condition-${rows[1].id}", useUnmergedTree = true).assertExists()
            assertEquals(listOf("one", "three"), vm.uiState.value.draft.rules.single().conditions.map { (it as CleanerCondition.Text).value })
            mainClock.advanceTimeBy(1000)
            mainClock.autoAdvance = true
            onNodeWithTag("condition-${rows[1].id}", useUnmergedTree = true).assertDoesNotExist()
            onNodeWithTag("condition-edit-${rows[2].id}").performClick()
            onNode(hasSetTextAction() and hasAnyAncestor(hasTestTag("condition-${rows[2].id}")), useUnmergedTree = true).performTextReplacement("edited")
            assertEquals(listOf("one", "edited"), vm.uiState.value.draft.rules.single().conditions.map { (it as CleanerCondition.Text).value })
    ...
    @Test fun idleStepsAndOverviewReturnAfterSelectionCloses() = runDesktopComposeUiTest(width = 800, height = 650) {
        ...
        onNodeWithTag("cleaner-rules-overview").assertDoesNotExist()
        runOnIdle { state = state.copy(phase = CleanerPhase.Idle, items = listOf(CleanerItemUi("build", "/root/build", "build", 100, 0, true, true))) }
        onNodeWithTag("cleaner-rules-overview").assertDoesNotExist()
        onNodeWithTag("cleaner-toolbar-rules").assertExists()
    ...
    @Test fun nativeWindowCanBeResizedAndTitleBarCloseRespectsSaving() = runDesktopComposeUiTest(
    ...
    @Test fun captureMaterialRulesEditorLightAndDark() = captureEditor(false)
    @Test fun captureEnglishMaterialRulesEditor() = captureEditor(true)
    ...
    private fun captureEditor(english: Boolean) {
        ...
        try {
            ...
            waitUntil { !vm.uiState.value.loading }
            runOnIdle {
                vm.edit { it.copy(rules = it.rules + CleanerRuleGroup("logs", if (english) "Large logs" else "大型日志", target = CleanerTarget.FILE, enabled = false, conditions = listOf(
                    CleanerCondition.Text(operator = TextOperator.ENDS_WITH, value = ".log"), CleanerCondition.FileSizeGreaterThan(104857600, "100", SizeUnit.MB)))) }
            }
            waitForIdle()
            ...
            onAllNodesWithContentDescription(if (english) "Expand rule" else "展开规则").assertCountEquals(2)
            val contentBounds = onNodeWithTag("cleaner-rules-window-content").fetchSemanticsNode().boundsInRoot
            assertTrue(kotlin.math.abs(contentBounds.bottom - 600f) <= 1f, "Editor bottom: $contentBounds")
            assertEquals(0f, contentBounds.top, 1f)
            assertEquals(0f, contentBounds.left, 1f)
            assertEquals(800f, contentBounds.right, 1f)
            ...
            fun capture(name: String) {
                ...
                output.resolve("$name$suffix.png").writeBytes(assertNotNull(Image.makeFromBitmap(bitmap).encodeToData()).bytes)
            }
            for (theme in listOf(false, true)) {
                runOnIdle { dark = theme }
                val mode = if (theme) "dark" else "light"
                capture("rules-$mode")
                onNode(hasContentDescription(if (english) "Expand rule" else "展开规则") and hasAnyAncestor(hasTestTag("rule-logs"))).performClick()
                ...
                onNode(hasContentDescription(if (english) "Collapse rule" else "折叠规则") and hasAnyAncestor(hasTestTag("rule-logs"))).performScrollTo().performClick()
                onNodeWithTag("cleaner-rule-list").performScrollToIndex(0)
                onNodeWithTag("cleaner-scan-settings-header", useUnmergedTree = true).performScrollTo().performClick()
