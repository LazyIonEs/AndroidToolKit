package org.tool.kit.tests.platform
...
class DesktopInteractionTest {
    ...
    @Test fun settingsInputsSurviveRecompositionThemeChangesAndNavigation() = runDesktopComposeUiTest(width = 800, height = 572) {
        ...
        path.performTextReplacement("test output path")
        suffix.performTextReplacement("-test-draft")
        for (index in 1..3) {
            runOnIdle {
            ...
            path.assertTextContains("test output path")
            suffix.assertTextContains("-test-draft")
        }
        ...
        setContent {
            TestContext {
                ...
                CompositionLocalProvider(LocalWindowInfo provides window) {
                    AppTheme(dark.value) { if (visible.value) CleanerRoute(current, useDarkTheme = dark.value) }
                }
