...
class JvmApkInformationRepositoryTest {
    ...
    @Test fun resourceTableSelectsTheHighestDensityIconAndDirectPathReadsTheRequestedEntry() = runBlocking {
    ...
    private suspend fun assertIconEntry(apk: File, entry: String, source: ApkIconSource) {
        val bytes = ZipFile(apk).use { zip -> zip.getInputStream(zip.getEntry(entry)).use { it.readBytes() } }
        assertContentEquals(bytes, source.bytes(), "Selected icon entry: $entry")
        val actual = decoder.decode(source)
        Image.makeFromEncoded(bytes).use { expected -> assertPixelsEqual(expected.toComposeImageBitmap(), actual) }
    }
