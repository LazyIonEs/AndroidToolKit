...
状态：**按用户更新的范围结束基线采集，进入 Phase 1**。

用户于 2026-09-09 最初选择严格逐阶段验收，随后明确要求不再记录真实键盘/拖拽、固定动画帧、录像和性能基线，直接进入 Phase 1。
以下保留已采集证据和原始缺口，未执行项不改标为通过；这些基线缺口不再阻止本次进入 Phase 1。
...
依用户最新指令直接执行 Phase 1 Koin；继续进行该阶段的编译、DI/owner 生命周期测试、九页 smoke 和本机打包验证。
