...
提取提交 `b6b3aa2a` 暂保留旧启动门控的初始 false，避免 IO bootstrap 在手动更新迁移时顺带启用网络。独立修复已通过两个真实 App 根组件测试（开启/关闭及后续重组），切换为等待 ready 后读取 `start_check_update`，每个窗口根会话只静默检查一次。该修复会让此前被初始 false 竞态跳过的启动检查开始运行，是明确记录的行为修复。
